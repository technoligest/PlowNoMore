...
export class Database {
  public static readonly instance = new Database();
  ...
  public getCustomerSummaries(): Promise<CustomerSummary[]> {
    return new Promise<CustomerSummary[]>((resolve, reject) => {
      const func = (err: any, connection: any) => {
        const customerSummaries: CustomerSummary[] = [];
        const requestLine = `SELECT name, id FROM customers`;
        const reqFunc = (error: Error, rowCount: number) => {
            resolve(customerSummaries);
            connection.release();
        };
        const request = new Request(requestLine, reqFunc);
        const rowHandler = (customerColumns: any) => {
  ...
  public getInvoice(invoiceId: string, callback: (invoice: Invoice) => void) {
    this.connectionPool.acquire((err: any, connection: any) => {
      const requestLine = `SELECT * FROM invoices WHERE id='${invoiceId}'`;
      const reqFunc = (error: Error, rowCount: number, rows: any[]) => {
          if (error) {
              console.log(err);
  ...
  public getInvoicesForCustomer(customerId: string, callback: (invoices: Invoice[]) => void) {
    this.connectionPool.acquire((error: any, connection: any) =>  {
      const requestLine = `SELECT * FROM invoices WHERE customerId='${customerId}'`;
  ...
  public addItem(itemToAdd: InvoiceItem) {
    this.connectionPool.acquire((error: any, connection: any) => {
      const requestLine = `INSERT INTO invoiceItems (invoiceId, description, cost)
                          VALUES ('${itemToAdd.invoiceId}', '${itemToAdd.description}', '${itemToAdd.cost}')`;
      const reqFunc = (err: Error, rowCount: number, rows: any[]) => {
        connection.release();
      };
      const request = new Request(requestLine, reqFunc);
  ...
  public addInvoice(invoiceToAdd: Invoice, callback: (addedInvoice: Invoice) => void) {
    this.connectionPool.acquire((error: any, connection: any) => {
      ...
      const requestLine = `INSERT INTO invoices (dateIssued, customerId, completed, paid)
                          VALUES ('${dateIssued}', '${invoiceToAdd.customerId}', ${isCompleted}, ${isPaid});
  ...
  public updateInvoice(invoiceToUpdate: Invoice, callback: (updatedInvoice: Invoice) => void) {
    this.connectionPool.acquire((error: any, connection: any) => {
      ...
      const requestLine = `UPDATE invoices
                          SET dateIssued='${dateIssued}', completed=${isCompleted}, paid=${isPaid}
  ...
  public addUserIfNotExist(accessToken: string, refreshToken: string, profile: Profile): Promise<Customer> {
    return new Promise((resolve, reject) => {
      resolve();
    });
  }

  private parseRow<T>(rowColumns: ColumnValue[]): T {
  ...
  private parseInvoiceRow(rowColumns: ColumnValue[]): Invoice {
    const parsedInvoice: Invoice = this.parseRow<Invoice>(rowColumns);
    parsedInvoice.items = [];
    return parsedInvoice;
  }

  private parseCustomerRow(rowColumns: ColumnValue[]): Customer {
